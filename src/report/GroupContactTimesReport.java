 ...
 */
public class GroupContactTimesReport extends Report implements ConnectionListener {
	protected HashMap<ConnectionInfo, ConnectionInfo> connections;
	private HashMap<ConnectionInfo, Integer> connectionStat = new HashMap<>();
	private HashMap<String, Integer> connectionTypeStat = new HashMap<>();
	 ...
	 * Constructor.
	 */
	public GroupContactTimesReport() {
	...
	public void hostsDisconnected(DTNHost host1, DTNHost host2) {
		...
		if (ci == null) {
			...
			/* If connectionTime lasts more than minimum Duration, record it.
			 * If two nodes meet more than 1, update the counter. */
			if (ci.getConnectionTime() > minDuration) {
				//update the count
				if (connectionStat.containsKey(ci)) {
					int oldValue = connectionStat.get(ci);
					connectionStat.put(ci, oldValue + 1);
				} else {
					connectionStat.put(ci, 1);
				}
			}
		}
	 ...
	 */
	protected void countStatistic(){
		for (Map.Entry<ConnectionInfo,Integer> entry : connectionStat.entrySet()) {
			System.out.println(entry.getKey() + "/" + entry.getValue());

			ConnectionInfo key = entry.getKey();
			String connType = key.getConnectionType(key);
			Integer count = entry.getValue();

			String keyStat = connType+ "[" +count+ "]";

			if (connectionTypeStat.containsKey(keyStat)) {
				int oldValue = connectionTypeStat.get(keyStat);
				connectionTypeStat.put(keyStat, oldValue + 1);
			} else {
				connectionTypeStat.put(keyStat, 1);
			}
		}
		for (Map.Entry<String,Integer> entry : connectionTypeStat.entrySet()) {
			System.out.println(entry.getKey() + "/" + entry.getValue());
		}
	}
	...
	public void done() {
		...
		//statistic
		countStatistic();
		write("#Host1.group<->Host2.group[Number of meetings]/Occurrence");
		for (Map.Entry<String,Integer> entry : connectionTypeStat.entrySet()) {
	...
	protected class ConnectionInfo {
		...
		/**
		 * Connection Type e.g. in group, between group
		 * @return h1.groupID & h2.groupID
		 */
		public String getConnectionType(ConnectionInfo conn){
			//Type: Master,Bachelor,Staff
			String h1Type = conn.h1.toString().replace(""+conn.h1.getAddress(),"");
			String h2Type = conn.h2.toString().replace(""+conn.h2.getAddress(),"");
			//sort group ID alphabetically
			int compare = h1Type.compareTo(h2Type);
			if (compare < 0){
				return (h1Type+"<->"+h2Type);
			} else {
				return (h2Type+"<->"+h1Type);
			}
		}
	}
}
